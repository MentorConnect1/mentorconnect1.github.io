...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .modal-content { background: white; border-radius: var(--radius-xl); box-shadow: 0 24px 60px rgba(0,0,0,.2); width: 100%; max-width: 400px; padding: 2rem; animation: slideIn .22s ease both; max-height: 90vh; overflow-y: auto; position: relative; }
    ...
    @media (max-width: 480px) { .auth-inner { max-width: 100%; } .card-header, .card-body { padding-left: 1.3rem; padding-right: 1.3rem; } .grid-2 { grid-template-columns: 1fr; } .verify-digit { width: 44px; height: 54px; font-size: 1.4rem; } }
  </style>
</head>
<body>
      ...
      <div class="card anim-slide">
        <div class="card-body" style="display:flex;flex-direction:column;gap:.7rem;">
          <button class="btn btn-primary" onclick="showPage('login')">
            <svg viewBox="0 0 24 24"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><path d="M10 17 L15 12 L10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>
            Log In
          </button>
      ...
      <div class="card anim-slide">
        ...
        <div class="card-body">
          <button class="role-option" data-role="mentor" onclick="selectRole('mentor')">
            <div class="role-option-icon"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg></div>
            <div class="role-option-text"><h3>Mentor</h3><p>Guide and teach others</p></div>
            <div class="role-check"><svg viewBox="0 0 24 24"><path d="M20 6 L9 17 L4 12"/></svg></div>
          </button>
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <div class="input-wrap">
              ...
              <button class="input-eye" onclick="togglePwd('signup-password',this)"><svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
            </div>
          </div>
          <div class="form-group">
...
<div id="page-verify" class="page">
  <div class="auth-wrap">
    <div class="auth-inner">
      <div class="logo-block">
        <div class="logo-icon"><svg viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M22 6 L12 13 L2 6"/></svg></div>
      ...
      <div class="card anim-slide">
        <div class="card-body">
          <div class="verify-hint">
            ...
            <span>Demo mode: your 6-digit code is shown below</span>
          </div>
          <div class="verify-code-display" id="verify-code-display">——————</div>
  ...
  <div class="app-layout">
    <div class="top-bar">
      <div class="top-bar-inner">
        <div class="top-bar-title"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg><h1>Mentors</h1></div>
        ...
        <div class="filter-row">
          <select class="form-select" id="mentor-location-filter" onchange="filterMentors()"><option value="">All Locations</option></select>
          <select class="form-select" id="mentor-school-filter" onchange="filterMentors()"><option value="">All Schools</option></select>
        </div>
      </div>
    </div>
...
<div id="page-messages" class="page">
  <div class="app-layout">
    <div class="top-bar">
      <div class="top-bar-inner">
        <div class="top-bar-title"><svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg><h1>Messages</h1></div>
      </div>
    </div>
...
<div id="page-chat" class="page">
  <div class="chat-wrap">
    <div class="chat-header">
      <button class="btn-back" onclick="showPage('messages')" style="margin-bottom:0"><svg viewBox="0 0 24 24"><path d="M15 18 L9 12 L15 6"/></svg></button>
...
<div id="page-resource-detail" class="page">
  <div class="app-layout">
    <div class="top-bar">
      <div class="top-bar-inner">
        <button class="btn-back" onclick="showPage('resources')"><svg viewBox="0 0 24 24"><path d="M15 18 L9 12 L15 6"/></svg>Back</button>
      </div>
    </div>
        ...
        <div class="settings-card-body">
          <div class="profile-top">
            ...
            <div class="profile-top-info">
              ...
              <small><svg viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/></svg><span id="settings-email"></span></small>
              <div class="role-tag" id="settings-role-tag"></div>
            </div>
          </div>
          <div class="grid-2">
            <div class="form-group"><label class="form-label">First Name</label><input id="settings-first" type="text" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Last Name</label><input id="settings-last" type="text" class="form-input" /></div>
          </div>
          <div class="form-group">
          ...
          <div class="toggle-row" id="settings-avail-row">
            <div class="toggle-row-text"><p>Available for hire</p><small>Let others know you're available</small></div>
            <label class="toggle"><input type="checkbox" id="settings-available" /><span class="toggle-slider"></span></label>
          </div>
          <div id="settings-tabroom-section" style="display:none">
            ...
            <button class="btn btn-outline btn-sm" style="margin-bottom:.9rem;width:auto" onclick="linkTabroom()">Link / Update Tabroom</button>
          </div>
          <button class="btn btn-primary" onclick="saveSettings()"><svg viewBox="0 0 24 24"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/></svg>Save Changes</button>
        </div>
      </div>
      <div class="settings-card">
        ...
        <div class="settings-card-body">
          <div class="account-item"><div><p>Email Verified</p><small id="settings-email-sub"></small></div><svg style="width:20px;height:20px;stroke:#16a34a;fill:none;stroke-width:2.5" viewBox="0 0 24 24"><path d="M20 6 L9 17 L4 12"/></svg></div>
          ...
          <button class="btn btn-danger-outline" onclick="promptDeleteAccount()" style="margin-top:.6rem"><svg viewBox="0 0 24 24"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg>Delete Account</button>
        </div>
      </div>
      <div class="settings-card" id="admin-panel" style="display:none">
        <div class="settings-card-header"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/></svg><h2>Admin Controls</h2></div>
        <div class="settings-card-body">
          <div class="form-group"><input type="text" class="form-input" id="admin-search-users" placeholder="Search by name or email..." oninput="renderAdminPanel()" /></div>
...
<div id="create-resource-modal" class="modal-overlay" onclick="if(event.target===this)closeCreateResourceModal()">
  <div class="modal-content">
    ...
    <div class="form-group"><label class="form-label">Title *</label><input id="new-res-title" type="text" class="form-input" /></div>
    <div class="form-group"><label class="form-label">Description / Content</label><textarea id="new-res-desc" class="form-textarea"></textarea></div>
    ...
    <div class="grid-2">
      <div class="form-group">
        ...
        <select id="new-res-cat" class="form-select"><option value="">Select</option><option value="debate">Debate</option><option value="public_speaking">Public Speaking</option><option value="coaching">Coaching</option><option value="judging">Judging</option><option value="general">General</option></select>
      </div>
      <div class="form-group">
        <label class="form-label">Type *</label>
        <select id="new-res-type" class="form-select"><option value="">Select</option><option value="document">Document</option><option value="video">Video</option><option value="link">Link</option><option value="default">Other</option></select>
      </div>
    </div>
...
// AUTOCOMPLETE DATA
// ═══════════════════════════════════════════════
const LOCATIONS = ['Atlanta, GA','Austin, TX','Baltimore, MD','Boston, MA','Charlotte, NC','Chicago, IL','Cleveland, OH','Columbus, OH','Dallas, TX','Denver, CO','Detroit, MI','Fort Worth, TX','Houston, TX','Indianapolis, IN','Jacksonville, FL','Kansas City, MO','Las Vegas, NV','Los Angeles, CA','Louisville, KY','Memphis, TN','Miami, FL','Milwaukee, WI','Minneapolis, MN','Nashville, TN','New Orleans, LA','New York, NY','Oklahoma City, OK','Orlando, FL','Philadelphia, PA','Phoenix, AZ','Pittsburgh, PA','Portland, OR','Raleigh, NC','Richmond, VA','Sacramento, CA','Salt Lake City, UT','San Antonio, TX','San Diego, CA','San Francisco, CA','San Jose, CA','Seattle, WA','St. Louis, MO','Tampa, FL','Tucson, AZ','Virginia Beach, VA','Washington, DC','Wichita, KS','Buffalo, NY','Cincinnati, OH','Fresno, CA'];
const SCHOOLS = ['Auburn University','Boston College','Boston University','Brown University','California Institute of Technology','Carnegie Mellon University','Columbia University','Cornell University','Dartmouth College','Duke University','Emory University','Florida State University','Georgetown University','Georgia Institute of Technology','Harvard University','Indiana University','Johns Hopkins University','Massachusetts Institute of Technology','Michigan State University','New York University','Northwestern University','Ohio State University','Penn State University','Princeton University','Purdue University','Rice University','Stanford University','Tufts University','Tulane University','UC Berkeley','UCLA','UC San Diego','University of Arizona','University of Chicago','University of Colorado','University of Florida','University of Georgia','University of Illinois','University of Maryland','University of Michigan','University of Minnesota','University of North Carolina','University of Notre Dame','University of Pennsylvania','University of Southern California','University of Texas','University of Virginia','University of Washington','Vanderbilt University','Wake Forest University','Washington University in St. Louis','Yale University','Bronx Science','Brooklyn Technical High School','Exeter Academy','Harvard-Westlake School','Horace Mann School','Lakeside School','Lexington High School','Loyola High School','Middlesex School','Milton Academy','North Carolina School of Science','Northside College Prep','Palo Alto High School','Phillips Academy Andover','Regis High School','Ridgewood High School','Stuyvesant High School','The Harker School','Thomas Jefferson High School','Trinity School','Westlake High School','Whitney Young High School'];
...
// ═══════════════════════════════════════════════
// SUPABASE DATA LAYER - All reads/writes go here
// ═══════════════════════════════════════════════
...
async function dbGetUsers() {
  ...
  try {
    ...
    return data || [];
  } catch(e) { console.error('dbGetUsers:', e); return []; }
}

async function dbGetUser(email) {
  if (!supa) return null;
  try {
    const { data, error } = await supa.from('users').select().eq('email', email).single();
...
async function dbUpsertUser(user) {
  ...
  try {
    const { error } = await supa.from('users').upsert(user, { onConflict: 'email' });
    if (error) throw error;
    return true;
  } catch(e) { console.error('dbUpsertUser:', e); return false; }
}

async function dbGetResources() {
  if (!supa) return DEMO_RESOURCES;
  try {
    const { data, error } = await supa.from('resources').select();
    if (error) throw error;
    return (data && data.length > 0) ? data : DEMO_RESOURCES;
  } catch(e) { return DEMO_RESOURCES; }
}

async function dbUpsertResource(res) {
  if (!supa) return false;
  try {
    const { error } = await supa.from('resources').upsert(res);
    if (error) throw error;
    return true;
  } catch(e) { console.error('dbUpsertResource:', e); return false; }
}

async function dbDeleteResource(id) {
  if (!supa) return false;
  try {
    const { error } = await supa.from('resources').delete().eq('id', id);
    if (error) throw error;
    return true;
  } catch(e) { console.error('dbDeleteResource:', e); return false; }
}

async function dbDeleteUser(email) {
  if (!supa) return false;
  try {
    const { error } = await supa.from('users').delete().eq('email', email);
    if (error) throw error;
    return true;
  } catch(e) { console.error('dbDeleteUser:', e); return false; }
}

// Conversations and messages stay in localStorage since they're already private
// and Supabase doesn't have those tables set up
function saveConvos(c) { try { localStorage.setItem('dc_convos', JSON.stringify(c)); } catch(e){} }
function loadConvos() { try { return JSON.parse(localStorage.getItem('dc_convos') || '[]'); } catch(e) { return []; } }
function saveMsgs(m) { try { localStorage.setItem('dc_msgs', JSON.stringify(m)); } catch(e){} }
function loadMsgs() { try { return JSON.parse(localStorage.getItem('dc_msgs') || '{}'); } catch(e) { return {}; } }
function saveNotifs(n) { try { localStorage.setItem('dc_notifs', JSON.stringify(n)); } catch(e){} }
function loadNotifs() { try { return JSON.parse(localStorage.getItem('dc_notifs') || '[]'); } catch(e) { return []; } }
function saveCurrentUser(u) { try { if (u) localStorage.setItem('dc_current', JSON.stringify(u)); else localStorage.removeItem('dc_current'); } catch(e){} }
function loadCurrentUser() { try { const d = localStorage.getItem('dc_current'); return d ? JSON.parse(d) : null; } catch(e) { return null; } }
...
async function init() {
  ...
  if (saved && saved.email_verified) {
    // Re-fetch from Supabase to get freshest data
    const fresh = await dbGetUser(saved.email);
    ...
    // If not found remotely (Supabase might be down), fall back to cached
    if (!supa) {
...
function acSearch(inputId, dropdownId, list) {
  ...
  const matches = list.filter(i => i.toLowerCase().includes(q)).slice(0, 8);
...
function acKeydown(e, dropdownId, inputId) {
  ...
  else if (e.key === 'Enter' && (acFocusIdx[dropdownId]||0) >= 0) { e.preventDefault(); const focused = items[acFocusIdx[dropdownId]]; if (focused) { document.getElementById(inputId).value = focused.textContent; closeAc(dropdownId); } }
...
async function handleLogin() {
  ...
  // *** Always query Supabase — never fallback to local storage for auth ***
...
async function handleSignup() {
  ...
  // *** Check Supabase for duplicate email — works across all devices ***
  const existing = await dbGetUser(email);
  if (existing) {
    btn.disabled = false;
    btn.innerHTML = 'Create Account & Verify Email';
...
async function verifyCode() {
  ...
  // *** Save verified user to Supabase — visible to ALL devices immediately ***
  const ok = await dbUpsertUser(state.pendingUser);
  if (!ok && supa) {
    errEl.textContent = 'Failed to save account. Check your connection and try again.';
    errEl.style.display = 'block';
    return;
  }
...
function resendCode() {
  ...
  errEl.textContent = 'New code generated!';
  errEl.style.display = 'block';
...
async function loadAppData() {
  // *** Always load users from Supabase — cross-device ***
  state.users         = await dbGetUsers();
  state.resources     = await dbGetResources();
...
// ═══════════════════════════════════════════════
const NAV_ITEMS = [
  { id:'mentors',       label:'Mentors',       page:'mentors',       svg:`<svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>` },
...
function buildNav(containerId, activePage) {
  ...
  container.innerHTML = `<div class="bottom-nav-inner">` + NAV_ITEMS.map(item => {
...
function renderMentors() {
  ...
  locSel.innerHTML = `<option value="">All Locations</option>` + locs.map(l=>`<option value="${escHtml(l)}">${escHtml(l)}</option>`).join('');
  schSel.innerHTML = `<option value="">All Schools</option>` + schools.map(s=>`<option value="${escHtml(s)}">${escHtml(s)}</option>`).join('');
  filterMentors();
}
...
function renderPersonList(containerId, people) {
  ...
  el.innerHTML = people.map(p => {
    ...
    const tabroomBadge = (p.role === 'judge' && p.tabroom_linked) ? `<span class="avail-badge avail-yes" style="margin-left:.35rem">✓ Tabroom</span>` : '';
      ...
      <div class="person-card-inner">
        ...
        <div class="person-info">
          ...
          <div class="person-meta">
            ...
            ${p.school?`<span class="meta-item"><svg viewBox="0 0 24 24"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/></svg>${escHtml(p.school)}</span>`:''}
          </div>
...
function startConvo(targetEmail) {
  ...
  const existing  = allConvos.find(c => c.participants.includes(state.currentUser.email) && c.participants.includes(targetEmail));
...
function renderMessages() {
  ...
  el.innerHTML = [...state.conversations].sort((a,b) => new Date(b.last_message_date) - new Date(a.last_message_date)).map(c => {
...
function renderResources() {
  document.getElementById('admin-create-resource-btn').style.display =
    (state.currentUser && state.currentUser.role === 'admin') ? 'block' : 'none';
  filterResources();
}
      ...
      <div class="resource-inner">
        ...
        <div class="resource-body">
          <div class="resource-title-row">
            ...
            ${r.url&&r.url!=='#'?`<a href="${r.url}" target="_blank" class="resource-link" onclick="event.stopPropagation()"><svg viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><path d="M15 3h6v6"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>`:''}
          </div>
    ...
    <div style="max-width:800px;margin:0 auto">
      <div style="display:flex;gap:1.5rem;margin-bottom:2rem">
        ...
        <div style="flex:1">
          ...
          <small style="color:#666">${r.posted_by_name?`By ${escHtml(r.posted_by_name)} • `:''}${dateStr}</small>
        </div>
      </div>
      ...
      ${isAdmin ? `<div style="border-top:1px solid var(--blue-200);padding-top:1.5rem;margin-top:2rem">
        ...
        <div class="form-group"><label class="form-label">Description</label><textarea id="edit-res-desc" class="form-textarea">${escHtml(r.description||'')}</textarea></div>
        <div class="form-group"><label class="form-label">Link / URL</label><input id="edit-res-url" type="text" class="form-input" value="${escHtml(r.url||'')}" /></div>
        <div class="grid-2">
        ...
        <div style="display:flex;gap:.6rem;margin-top:1.2rem">
          ...
          <button class="btn btn-danger-outline" onclick="deleteResource('${r.id}')">Delete</button>
        </div>
      </div>` : ''}
    </div>`;
  buildNav('bottom-nav-resource-detail','resources');
...
async function saveResourceEdit() {
  ...
  await dbUpsertResource(r);
  openResourceDetail(r.id);
}

async function deleteResource(resourceId) {
  if (!confirm('Delete this resource? This cannot be undone.')) return;
  await dbDeleteResource(resourceId);
  state.resources = state.resources.filter(r => r.id !== resourceId);
  showPage('resources');
  renderResources();
}

function showCreateResourceModal() { document.getElementById('create-resource-modal').classList.add('open'); }
...
async function createNewResource() {
  ...
  await dbUpsertResource(newResource);
  ...
  el.innerHTML = state.notifications.map(n => {
    ...
    return `<div class="notif-card ${!n.read?'unread':''}" onclick="handleNotifClick('${n.id}')">
      <div class="notif-inner">
        <div class="notif-icon"><svg viewBox="0 0 24 24"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/></svg></div>
...
function renderSettings() {
  ...
  else { availRow.style.display = 'flex'; document.getElementById('settings-available').checked = u.available_for_hire !== false; }
...
function renderTabroomStatus() {
  ...
  el.innerHTML = u.tabroom_linked
    ? `<div class="tabroom-status linked"><svg viewBox="0 0 24 24"><path d="M20 6 L9 17 L4 12"/></svg>Linked: <strong>${escHtml(u.tabroom_username)}</strong></div>`
    : `<div class="tabroom-status unlinked"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/></svg>No Tabroom account linked</div>`;
}

async function linkTabroom() {
  const username = document.getElementById('settings-tabroom').value.trim();
  if (!username) { const e = document.getElementById('settings-error'); e.textContent='Enter a Tabroom username first.'; e.style.display='block'; setTimeout(()=>e.style.display='none',3000); return; }
  ...
  s.querySelector('span').textContent = `Tabroom "${username}" linked!`;
...
async function saveSettings() {
  ...
  const s = document.getElementById('settings-success');
  s.querySelector('span').textContent = 'Profile updated!';
  s.style.display = 'flex'; setTimeout(()=>s.style.display='none', 3000);
}

async function persistCurrentUserChanges() {
  saveCurrentUser(state.currentUser);
  // *** Update Supabase so other devices see the latest profile ***
  await dbUpsertUser(state.currentUser);
  // Also update in local state.users array
  const idx = state.users.findIndex(u => u.email === state.currentUser.email);
  if (idx >= 0) state.users[idx] = state.currentUser;
  else state.users.push(state.currentUser);
}
...
async function promptDeleteAccount() {
  ...
  await dbDeleteUser(state.currentUser.email);
  saveCurrentUser(null);
  state.currentUser = null;
  showPage('landing');
}
...
function renderAdminPanel() {
  ...
  document.getElementById('admin-users-list').innerHTML = filtered.length
    ? filtered.map(u => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem;border:1px solid #e5e7eb;border-radius:var(--radius);margin-bottom:.6rem">
        ...
        <button class="btn btn-sm btn-danger-outline" onclick="adminDeleteUser('${u.email.replace(/'/g,"\\'")}')">Delete</button>
      </div>`).join('')
...
async function changeUserRole(email, newRole) {
  ...
  if (state.currentUser.email === email) { state.currentUser.role = newRole; saveCurrentUser(state.currentUser); }
  renderAdminPanel();
}
...
async function adminDeleteUser(email) {
  ...
  if (state.currentUser.email === email) { saveCurrentUser(null); state.currentUser = null; showPage('landing'); return; }
  renderAdminPanel();
}
